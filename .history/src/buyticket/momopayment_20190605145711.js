...
export default class MomoPayment extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      textAmount: this.formatNumberToMoney(route.price, null, "")
    });
  ...
  async handlePress(token, phone, amount) {
    ...
    fetch("http://192.168.43.177:8000/api/ticket/buy", {
      method: "POST",
      headers: {
      ...
      .then(responseJson => {
        ...
        this.props.navigation.navigate("BuySuccess",{codeview: responseJson.codeview};
        this.setState({
          description: "Giao dịch thành công: " + responseJson.codeview,
          processing: false,
          codeview: responseJson.codeview
        });
      })
      .catch(error => {
        console.error(error);
